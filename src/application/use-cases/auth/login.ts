...
import {AuthException} from '@/application/error-patterns';
...
export default class Login {
    ...
    async execute(input: Input): Promise<Output> {
        ...
        if (!user) throw new AuthException(Login.errorCodes.INVALID_CREDENTIALS);
        if (!await this.isValidPassword(input.password, user.password)) throw new AuthException(Login.errorCodes.INVALID_CREDENTIALS);
        const player = await this.playerRepository.findByUserId(user.id);
        if (!player) throw new AuthException(Login.errorCodes.PLAYER_NOT_EXISTS);
        const payload = {
            sub: user.id,
            email: user.email,
    ...
    private async isValidPassword(password: string, hash: string) {
        return await this.hashingService.compare(password, hash);
    };
