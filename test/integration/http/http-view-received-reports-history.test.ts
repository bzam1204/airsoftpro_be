...
describe('Ver Histórico de Denúncias Recebidas por HTTP', function () {
    ...
    beforeAll(function () {
        ...
        const reportsList = new Array(10).fill(null).map((_, i) => new Report({
            ...reportProps, id: `${++i}`, gameId: `${i}`, to: '1'
        }));
    ...
    it('Deve visualizar todas as denúncias recebidas por um jogador', async function () {
        await request(app.getInstance())
            .get('/report/received/1')
            .expect(200)
            .expect(res => {
    ...
    it('Deve retornar lista vazia quando o jogador não tiver denúncias recebidas', async function () {
